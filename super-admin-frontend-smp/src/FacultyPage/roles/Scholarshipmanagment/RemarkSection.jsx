...
const RemarkSection = () => {
  ...
  const mergedStudents = students
    ...
    .filter((student) => student.scholarshipStatus === "Yes" && student.pdfs.length > 0); // Only students with remarks
        ...
        <div className={`${currentTheme.cardBg} rounded-2xl shadow-2xl p-6 md:p-8 ${currentTheme.cardBorder} transition-all duration-300 animate-fade-in`}>
          ...
          {loading ? (
            <div className="flex items-center justify-center h-40 animate-fade-in">
              ...
              <span className={`ml-2 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>Loading remarks...</span>
            </div>
                  ...
                  {filteredStudents.map((student, index) =>
                    student.pdfs.map((pdf, pdfIndex) => (
                      ...
                      >
                        ...
                        <td className={`px-4 py-2 ${currentTheme.textPrimary}`}>{pdf.remark}</td>
