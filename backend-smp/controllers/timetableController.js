  ...
  try {
    ...
    if (req.user && req.user.type === "student") {
      ...
      const deptPatterns = [
        ...
        studentDept.replace(/eletronic enigneering/gi, 'electronic engineering'), // fix both
      ];
      
      // Create regex that matches any of the patterns
      const regexPattern = deptPatterns.join('|');
      filter["collegeInfo.department"] = { $regex: regexPattern, $options: "i" };
      ...
      if (req.user.section && req.user.section.length <= 3 && req.user.section.match(/^[A-Za-z0-9]+$/)) {
        filter["collegeInfo.section"] = { $regex: req.user.section, $options: "i" };
      }
      ...
      if (req.user.role === "principal" || req.user.role === "Principal" || req.user.type === "principal" || req.user.type === "Principal") {
        ...
        console.log("Faculty/General filter by department:", req.query.department);
      }
...
const createTimetable = async (req, res) => {
  try {
    ...
    if (!collegeInfo || !subjects || !timetableData || !timeSlots) {
      return res.status(400).json({ 
        success: false,
        error: "Missing required fields: collegeInfo, subjects, timetableData, timeSlots" 
      });
    }

    // Validate collegeInfo structure
    if (!collegeInfo.department || !collegeInfo.semester || !collegeInfo.section) {
      return res.status(400).json({ 
        success: false,
        error: "Missing required collegeInfo fields: department, semester, section" 
      });
    }
    ...
    if (existingTimetable) {
      return res.status(409).json({
        success: false,
        error: "Timetable already exists for this department, semester, and section"
      });
    }
