...
const facultyAttendanceData = [
  { section: "Section A", attendance: 92, faculty: "Dr. Smith", subject: "Mathematics" },
  { section: "Section B", attendance: 88, faculty: "Prof. Johnson", subject: "Physics" },
  { section: "Section C", attendance: 95, faculty: "Dr. Williams", subject: "Chemistry" },
  { section: "Section D", attendance: 85, faculty: "Prof. Brown", subject: "Biology" },
];
...
export default function HodDashboard() {
  ...
  const fetchTodos = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/dashboard/hod-todos?status=${todoFilter === 'all' ? '' : todoFilter}`,
        {
          headers: {
  ...
  const handleCreateTodo = async () => {
    try {
      ...
      const response = await fetch(
        "http://localhost:5000/api/dashboard/hod-todos",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleUpdateTodo = async (todoId, status, progress) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/dashboard/hod-todos/${todoId}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const response = await fetch(
          "http://localhost:5000/api/dashboard/hod-stats",
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
  ...
  const statsCards = [
    ...
    },
    {
      ...
      value: loading ? "..." : error ? "Error" : (todoStats.pending + todoStats.inProgress),
    ...
    },
    {
      ...
      value: loading ? "..." : error ? "Error" : todoStats.completed,
                      ...
                      <div>
                        ...
                        <div className="flex items-center mt-2">
                          ...
                          <span className="text-sm text-gray-500 ml-1">
                            {card.title === "Tasks Remaining" && todoStats.overdue > 0 ? "overdue" : "this week"}
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center">
                      ...
                      <h3 className="text-xl font-semibold text-gray-900">
                        {filter === "students" ? "Student Attendance" : "Faculty Attendance"} by Section
                      </h3>
                    </div>
                    <select
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center">
                      ...
                      <h3 className="text-xl font-semibold text-gray-900">Department Todos</h3>
                    </div>
                  ...
                  <div className="grid grid-cols-5 gap-2 mb-4">
                    <div className="bg-gray-50 rounded p-2 text-center">
                      <p className="text-lg font-bold text-gray-900">{todoStats.total}</p>
                      <p className="text-xs text-gray-600">Total</p>
                    </div>
                    <div className="bg-yellow-50 rounded p-2 text-center">
                      <p className="text-lg font-bold text-yellow-600">{todoStats.pending}</p>
                      <p className="text-xs text-yellow-600">Pending</p>
                    </div>
                    <div className="bg-blue-50 rounded p-2 text-center">
                      <p className="text-lg font-bold text-blue-600">{todoStats.inProgress}</p>
                      <p className="text-xs text-blue-600">Progress</p>
                    </div>
                    <div className="bg-green-50 rounded p-2 text-center">
                      <p className="text-lg font-bold text-green-600">{todoStats.completed}</p>
                      <p className="text-xs text-green-600">Done</p>
                    </div>
                    <div className="bg-red-50 rounded p-2 text-center">
                      <p className="text-lg font-bold text-red-600">{todoStats.overdue}</p>
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {todos.length === 0 ? (
                      ...
                      todos.slice(0, 6).map((todo) => (
                        <div key={todo._id} className="border border-gray-200 rounded p-3 hover:bg-gray-50 transition-colors">
                          <div className="flex items-start justify-between">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center space-x-2 mb-1">
                                <h4 className="font-medium text-sm text-gray-900 truncate">{todo.title}</h4>
                            ...
                            <div className="flex items-center space-x-1 ml-2">
                              {todo.status !== 'Completed' && (
                                <select
                                  ...
                                  value={todo.status}
                                  onChange={(e) => handleUpdateTodo(todo._id, e.target.value, e.target.value === 'Completed' ? 100 : todo.progress)}
                                >
                                  <option value="Pending">Pending</option>
                                  <option value="In Progress">In Progress</option>
                                  <option value="Completed">Completed</option>
