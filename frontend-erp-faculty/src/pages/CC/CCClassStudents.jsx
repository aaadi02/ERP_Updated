...
const CCClassStudents = ({ userData }) => {
  ...
  const fetchCCClassStudents = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:5000/api/faculty/get-cc-class-students', {
        method: 'GET',
        headers: {
      ...
      if (data.success && data.data) {
        ...
        setStats({
          ...
          activeStudents: data.data.students?.filter(s => s.status === 'active').length || 0,
          maleStudents: data.data.students?.filter(s => s.gender === 'Male').length || 0,
          femaleStudents: data.data.students?.filter(s => s.gender === 'Female').length || 0
        });
      }
    } catch (error) {
  ...
  const filteredStudents = students.filter(student => {
    ...
    const matchesSection = !filterSection || student.section?.toUpperCase() === filterSection.toUpperCase();
  ...
  const exportToCSV = () => {
    const headers = [
      'Enrollment Number', 'Name', 'Email', 'Phone', 'Year', 'Section', 
      'Department', 'Gender', 'Status', 'Attendance %', 'Address'
    ];
    ...
    a.href = url;
    a.download = `CC_Class_Students_${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const StatCard = ({ title, value, icon: Icon, color, description }) => (
    <div className={`${color} rounded-xl p-6 text-white shadow-lg hover:shadow-xl transition-all duration-300`}>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredStudents.map((student, index) => (
                    <tr key={student._id || index} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center">
                          ...
                          <div className="ml-4">
                            ...
                            <div className="text-sm text-gray-500">{student.enrollmentNumber || 'N/A'}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            Year {student.year || 'N/A'} - Section {student.section || 'N/A'}
                          </div>
                      ...
                      <td className="px-6 py-4">
                        <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getAttendanceColor(student.attendancePercentage || 0)}`}>
                          {student.attendancePercentage || 0}%
                        </div>
                      </td>
                      <td className="px-6 py-4">
